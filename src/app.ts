...
import { calorieGuidance, calorieTrend, dailyCalorieGuide, exerciseCalories, formatDate, kgToPounds, latestWeight, nutritionTargets, poundsToKg, round, shiftDate, totalsFor, weightProjection } from "./nutrition";
...
export class DaybookApp {
  ...
  private trend(): string {
    ...
    const goalLine = projection?.goalDate && goal != null
      ? `At this rate, you may reach ${fmt(this.displayWeight(goal), 1)} ${this.weightUnit()} around ${formatDate(projection.goalDate)}.`
      : goal != null ? "Your recent intake trend is not moving toward the goal yet." : "Add a goal weight to estimate a target date.";
    const forecast = projection ? `<section class="forecast card"><div class="forecast-copy"><span class="eyebrow">Your current pace</span><h2>${goalLine}</h2><p>Based on ${projection.activeDays} active food-log ${projection.activeDays === 1 ? "day" : "days"} across the last ${projection.spanDays} ${projection.spanDays === 1 ? "day" : "days"}.</p></div><div class="forecast-month"><span>In one month</span><strong>${fmt(this.displayWeight(projection.oneMonthWeightLb), 1)}</strong><small>${this.weightUnit()} estimated</small></div><div class="forecast-facts"><span><b>${fmt(projection.averageIntake)}</b> avg kcal eaten</span><span><b>${fmt(projection.averageExercise)}</b> exercise kcal/day</span><span><b>${fmt(Math.abs(this.displayWeight(projection.weeklyChangeLb) ?? 0), 2)}</b> ${this.weightUnit()}/week ${projectedDirection}</span></div><p class="forecast-note">A planning estimate—not a promise. Food portions, workout effort, metabolism, and water weight can move the result.</p></section>` : `<section class="forecast card forecast-empty"><span class="eyebrow">Your current pace</span><h2>Keep logging to unlock a forecast</h2><p>Add baseline details in Settings and log meals on a few active days.</p></section>`;
    const exerciseRows = exercises.length ? exercises.slice(0, 8).map((entry) => `<div class="history-row exercise-row"><span class="history-dot"></span><span>${formatDate(entry.date)} · ${html(EXERCISE_LABELS[entry.kind])} · ${fmt(entry.minutes)} min</span><strong>~${fmt(current ? exerciseCalories(entry.kind, entry.minutes, current) : null)} kcal</strong></div>`).join("") : `<div class="empty compact-empty">No exercise logged yet. A short dumbbell session or walk still counts.</div>`;
    return `<div class="head"><div><span class="eyebrow">Your trend</span><h1 class="title">Progress</h1><p class="subtitle">Small check-ins make the longer pattern visible.</p></div><div class="progress-actions"><button class="btn btn-icon" data-action="open-exercise" aria-label="Add exercise">${icon("Dumbbell")}<span>Add exercise</span></button><button class="btn-primary btn-icon" data-action="open-weight" aria-label="Check in weight">${icon("Weight")}<span>Check in</span></button></div></div>${forecast}<div class="metricgrid"><div class="metric"><span class="metric-icon">${icon("Weight")}</span><div class="metricv">${fmt(this.displayWeight(current), 1)}</div><div class="metricl">current ${this.weightUnit()}</div></div><div class="metric"><span class="metric-icon">${icon("Target")}</span><div class="metricv">${fmt(this.displayWeight(goal), 1)}</div><div class="metricl">goal ${this.weightUnit()}</div></div></div><div class="progress-history"><section class="section"><p class="label">Weight history</p>${weights.length ? weights.map((weight) => `<div class="history-row"><span class="history-dot"></span><span>${formatDate(weight.date)}</span><strong>${fmt(this.displayWeight(weight.weightLb), 1)} ${this.weightUnit()}</strong></div>`).join("") : `<div class="empty compact-empty">No check-ins yet.</div>`}</section><section class="section"><p class="label">Recent exercise</p>${exerciseRows}</section></div>`;
  }
