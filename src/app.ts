...
export class DaybookApp {
  ...
  private render(): void {
    this.root.innerHTML = `<div class="shell"><aside class="side"><div class="wordmark">daybook</div>${this.nav()}<div class="sidebottom">Private by default.<br>Stored in this browser.</div></aside><main class="main"><header class="top"><div class="wordmark">daybook</div><button class="btn" data-action="open-ai">AI bridge</button></header><div class="view">${this.content()}</div></main></div>${this.nav(true)}${this.modalHtml()}${this.state.profile.onboardingComplete ? "" : this.onboarding()}<div class="toast" id="toast"></div>`;
  }

  private nav(bottom = false): string {
    const items: [View, string][] = [["day", "day"], ["library", "library"], ["trend", "progress"], ["settings", "settings"]];
    const buttons = items.map(([id, label]) => `<button class="nav ${this.view === id ? "active" : ""}" data-action="view" data-view="${id}">${label}</button>`).join("");
  ...
  private day(): string {
    ...
    return `<div class="datebar"><button class="tiny-btn" data-action="date" data-days="-1" aria-label="Previous day">‹</button><div class="datecenter"><div class="datemain">${formatDate(date, true)}</div><button class="text-btn tiny" data-action="today">today</button></div><button class="tiny-btn" data-action="date" data-days="1" aria-label="Next day">›</button></div><section class="card summary"><div class="between"><div><div class="guide"><span class="big">${fmt(totals.calories)}</span><span class="tiny">${guide ? `/ ${fmt(guide)} kcal` : "kcal"}</span></div><div class="tiny">logged today</div></div><button class="btn-primary" data-action="choose-food">add food</button></div><div class="progress"><div style="width:${pct}%"></div></div><div class="macros">${this.macro("protein", totals.proteinG, targets?.proteinG)}${this.macro("carbs", totals.carbsG, targets?.carbsG)}${this.macro("fat", totals.fatG, targets?.fatG)}${this.macro("fiber", totals.fiberG, targets?.fiberG)}</div></section>${meals}`;
  }
  ...
  private meal(period: Period, date: string): string {
    ...
    return `<section class="mealgroup"><div class="mealhead"><div class="mealname">${period}</div><div class="mealsum">${fmt(total.calories)} kcal · ${fmt(total.proteinG, 1)}p · ${fmt(total.carbsG, 1)}c · ${fmt(total.fatG, 1)}f</div></div>${entries.length ? entries.map((entry) => `<div class="entry"><div class="entrymain"><div class="ename">${html(entry.nameSnapshot)}</div><div class="esub">${html(entry.servingSnapshot.description)} × ${fmt(entry.servings, 2)}</div></div><div class="entrymacro">${this.macroBar(entry.nutritionSnapshot.proteinG, entry.nutritionSnapshot.carbsG, entry.nutritionSnapshot.fatG)}</div><div class="row"><span class="ekcal">${fmt(entry.nutritionSnapshot.calories * entry.servings)} kcal</span><button class="tiny-btn" data-action="delete-entry" data-id="${entry.id}">remove</button></div></div>`).join("") : `<button class="mealempty" data-action="choose-food" data-period="${period}">+ add ${period}</button>`}</section>`;
  }
  ...
  private library(): string {
    const cards = [...this.state.foods].sort((a, b) => a.name.localeCompare(b.name)).map((food) => `<div class="library-card"><div class="between"><div class="grow"><div>${html(food.name)}</div><div class="tiny">${html(food.brand || food.serving.description)} · ${fmt(food.nutrition.calories)} kcal</div></div><div class="row"><button class="tiny-btn" data-action="log" data-id="${food.id}">log</button><button class="tiny-btn" data-action="edit-food" data-id="${food.id}">edit</button></div></div></div>`).join("");
    return `<div class="head"><div><h1 class="title">Food library</h1><p class="subtitle">Reusable serving-size nutrition records.</p></div><button class="btn-primary" data-action="new-food">new food</button></div>${cards || `<div class="empty">No saved foods yet. Create one here or ask an AI to structure a meal.</div>`}`;
  }
  ...
  private trend(): string {
    ...
    return `<div class="head"><div><h1 class="title">Progress</h1><p class="subtitle">Weight check-ins stay on this device.</p></div><button class="btn-primary" data-action="open-weight">check in</button></div><div class="metricgrid"><div class="metric"><div class="metricv">${fmt(this.displayWeight(current), 1)}</div><div class="metricl">current ${this.weightUnit()}</div></div><div class="metric"><div class="metricv">${fmt(this.displayWeight(goal), 1)}</div><div class="metricl">goal ${this.weightUnit()}</div></div></div><section class="section"><p class="label">history</p>${weights.length ? weights.map((weight) => `<div class="library-card between"><span>${formatDate(weight.date)}</span><span>${fmt(this.displayWeight(weight.weightLb), 1)} ${this.weightUnit()}</span></div>`).join("") : `<div class="empty">No check-ins yet.</div>`}</section>`;
  }

  private settings(): string {
    const profile = this.state.profile;
    const guidance = calorieGuidance(profile);
    return `<div class="head"><div><h1 class="title">Settings</h1><p class="subtitle">Plan, portability, and privacy.</p></div></div><form data-form="settings" class="card pad stack"><div class="two">${field("daily calorie guide", "manualDailyGuide", profile.manualDailyGuide ?? "", "number", "min=500 placeholder=automatic")}${field("activity multiplier", "activityPAL", profile.activityPAL, "number", "min=1.2 max=2.4 step=.1")}${field(`goal weight (${this.weightUnit()})`, "goalWeight", this.displayWeight(profile.goalWeightLb) ?? "", "number", "step=.1")}${field("weekly pace (lb)", "rateLbWeek", profile.rateLbWeek, "number", "min=0 step=.25")}</div><label class="field"><span>goal</span><select name="goalType"><option value="lose" ${profile.goalType === "lose" ? "selected" : ""}>lose</option><option value="maintain" ${profile.goalType === "maintain" ? "selected" : ""}>maintain</option><option value="gain" ${profile.goalType === "gain" ? "selected" : ""}>gain</option></select></label><div class="notice">${guidance.ok ? `Automatic estimate: ${fmt(guidance.target)} kcal/day${guidance.weeks ? ` · roughly ${fmt(guidance.weeks)} weeks` : ""}.` : html(guidance.reason)}</div><button class="btn-primary" type="submit">save plan</button></form><section class="section"><p class="label">data</p><div class="card settings"><button class="setting" data-action="open-ai"><span>AI bridge</span><span class="tiny">copy / paste</span></button><button class="setting" data-action="open-backup"><span>backup & restore</span><span class="tiny">portable JSON</span></button><button class="setting" data-action="onboard"><span>edit baseline</span><span class="tiny">profile setup</span></button></div></section>`;
  }

  private onboarding(): string {
    const p = this.state.profile;
    return `<div class="overlay show"><div class="onboard"><div class="otop"><div class="wordmark">daybook</div></div><div class="ocontent"><form data-form="onboarding"><h1 class="otitle">A small private record of your day.</h1><p class="ocopy">These basics create a starting guide. Nothing leaves this browser unless you export or copy it.</p><div class="two">${field("age", "age", p.age ?? "", "number", "min=18 max=120 required")}${field("height (inches)", "heightIn", p.heightIn ?? "", "number", "min=36 step=.1 required")}${field("current weight (lb)", "weightLb", p.weightLb ?? "", "number", "min=50 step=.1 required")}${field("goal weight (lb)", "goalWeightLb", p.goalWeightLb ?? "", "number", "min=50 step=.1")}</div><label class="field"><span>sex used by energy equation</span><select name="sex" required><option value="">choose</option><option value="female" ${p.sexForEquation === "female" ? "selected" : ""}>female</option><option value="male" ${p.sexForEquation === "male" ? "selected" : ""}>male</option></select></label><label class="field"><span>ordinary activity</span><select name="activity"><option value="1.4">mostly still</option><option value="1.6" selected>lightly moving</option><option value="1.8">regularly active</option><option value="2">very active</option><option value="2.2">exceptionally active</option></select></label><div class="notice">This is a planning aid, not medical advice.</div><div class="oactions"><span></span><button class="btn-primary">enter daybook</button></div></form></div></div></div>`;
  }
  ...
  private close(): string { return `<button class="close" data-action="close" aria-label="Close">×</button>`; }
  ...
  private download(): void { const url = URL.createObjectURL(new Blob([exportBackup(this.state)], { type: "application/json" })); const link = document.createElement("a"); link.href = url; link.download = `daybook-backup-${this.state.prefs.date}.json`; link.click(); URL.revokeObjectURL(url); }
